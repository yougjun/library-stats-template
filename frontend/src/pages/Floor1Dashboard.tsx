...
interface MonthlyData {
  ...
  ai_library_count: number
  ai_visitors: number
  ai_device_users: number
  has_data: boolean
  is_completed: boolean
  manual_status: boolean | null
}

export default function Floor1Dashboard() {
  ...
  useEffect(() => {
    ...
    socket.on('floor1_completion_status_updated', (data) => {
  ...
  const loadMonthlyData = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${apiUrl}/api/floor1/metadata/year/${currentYear}`)
      const metadataByMonth = response.data

      const monthlyData = Object.keys(metadataByMonth).sort().map(yearMonth => {
        const statusKey = `floor1_completion_${yearMonth}`
  ...
  const toggleCompletionStatus = async (yearMonth: string, currentStatus: boolean) => {
    ...
    try {
      const apiUrl = import.meta.env.VITE_API_URL || window.location.origin
      await axios.post(
        `${apiUrl}/api/floor1/completion-status/${yearMonth}`,
    ...
    },
    {
      ...
      render: (_: any, record: MonthlyData) => (
        <Space size="small" direction="vertical" style={{ fontSize: '12px' }}>
          ...
          <Space size="small">
            <span>출입자: {(record.ai_visitors || 0).toLocaleString()}</span>
            <span>기기이용자: {(record.ai_device_users || 0).toLocaleString()}</span>
          </Space>
        </Space>
      )
    },
      ...
      render: (_: any, record: MonthlyData) => (
        <Space>
          <Button
              ...
              size="small"
              icon={<EditOutlined />}
              onClick={() => navigate(`/floor1/input/${record.year_month}`)}
            >
              입력
            </Button>
          <Button
            ...
            onClick={() => navigate(`/prediction?month=${record.year_month}&floor=floor1`)}
          ...
          <Button
            ...
            onClick={() => navigate(`/statistics/floor1/${record.year_month}`)}
          >
            통계
          </Button>
        </Space>
  ...
  const chartData = data.map(item => ({
    ...
    출입자: item.ai_visitors || 0,
    기기이용자: item.ai_device_users || 0
  }))
    ...
    <div style={{ padding: 24 }}>
      ...
      <Card
        title={
          <Space>
            <CalendarOutlined />
            <span>1층 어린이자료실 - {currentYear}년 데이터 관리</span>
            <DatePicker
              picker="year"
              value={selectedYear}
        ...
        extra={
          <Space>
            ...
            <Button
              icon={<LineChartOutlined />}
              onClick={() => navigate(`/statistics/floor1/${getDefaultStatsMonth('floor1')}`)}
            >
              월별 통계
            </Button>
            <Button
              icon={<BarChartOutlined />}
              onClick={() => navigate(`/statistics/yearly-floor1/${currentYear}`)}
            >
              연간 통계
            </Button>
            <Button
              ...
              children: (
                <>
                  ...
                  <Row gutter={[16, 16]}>
                    <Col span={24}>
                      <ChartContainer
                        title="월별 미래형 도서관 현황 (출입자/기기이용자)"
                        chartId="ai-library-chart"
                        onMaximize={() => setLocalMaximizedChart('ai-library-chart')}
                      >
                        <ResponsiveContainer width="100%" height={250}>
                          <BarChart data={chartData}>
                            <CartesianGrid strokeDasharray="3 3" />
                            ...
                            <Tooltip />
                            <Legend />
                            <Bar dataKey="출입자" fill="#82ca9d" name="출입자(1일출입증+정회원)" />
                            <Bar dataKey="기기이용자" fill="#ff7c7c" name="기기이용자(책봇+에어프로젝션+핑거스토리+AR북+무인반납실)" />
                          </BarChart>
                        </ResponsiveContainer>
                      </ChartContainer>
                    </Col>
      ...
      <ChartMaximizedModal
        ...
        title={
          ...
          localMaximizedChart === 'ai-library-chart' ? '월별 미래형 도서관 현황 (출입자/기기이용자)' :
          '차트'
        }
      ...
      >
        ...
        {localMaximizedChart === 'ai-library-chart' && (
          <MaximizedChart
            type="bar"
            data={chartData}
            bars={[
              { dataKey: '출입자', fill: '#82ca9d', name: '출입자(1일출입증+정회원)' },
              { dataKey: '기기이용자', fill: '#ff7c7c', name: '기기이용자(책봇+에어프로젝션+핑거스토리+AR북+무인반납실)' }
            ]}
          />
        )}
      </ChartMaximizedModal>
